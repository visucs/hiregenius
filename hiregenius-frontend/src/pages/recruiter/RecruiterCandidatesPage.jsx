  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: 'min(440px, 100vw)', height: '100%', overflowY: 'auto', background: 'var(--bg-elevated)', borderLeft: '1px solid var(--border)', boxShadow: '-16px 0 64px rgba(0,0,0,0.18)' }}
      >
        {/* Header */}
        <div style={{ position: 'sticky', top: 0, zIndex: 10, backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)', background: 'rgba(252,251,247,0.92)', padding: '16px clamp(14px, 3vw, 22px)' }} className="dark:!bg-[rgba(15,24,5,0.92)]">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 42, height: 42, minWidth: 42, borderRadius: '50%', background: 'linear-gradient(135deg, #3D5016, #6B8A3A)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 15, fontWeight: 800, boxShadow: '0 3px 10px rgba(61,80,22,0.35)', flexShrink: 0 }}>
    ...
    <div style={{ background: 'var(--bg-base)', minHeight: '100%' }}>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 24 }}>
              <div>
                ...
                <h1 style={{ fontSize: 'clamp(22px, 4vw, 30px)', fontWeight: 900, color: '#fff', letterSpacing: '-0.04em', marginBottom: 6 }}>Candidates</h1>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', paddingBottom: 22 }}>
              <div style={{ position: 'relative', flex: '1 1 200px', minWidth: 'min(100%, 200px)', maxWidth: 340 }}>
                ...
                <input
                  ...
                  style={{ width: '100%', minHeight: 44, paddingLeft: 36, paddingRight: 34, paddingTop: 9, paddingBottom: 9, borderRadius: 11, fontSize: 13, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(107,138,58,0.30)', color: '#fff', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
                />
              </div>
                ...
                {FILTER_TABS.map((s) => {
                  ...
                  return (
                    <button key={s} onClick={() => setStatusFilter(s)} id={`candidates-filter-${s.toLowerCase()}`}
                      style={{ padding: '8px 14px', minHeight: 38, borderRadius: 9, fontSize: 12, fontWeight: 700, cursor: 'pointer', border: 'none', transition: 'all 0.15s', background: active ? (cfg?.color ?? '#fff') : 'transparent', color: active ? '#fff' : 'rgba(255,255,255,0.5)' }}
                    >
      ...
      <div style={{ padding: 'clamp(16px, 3vw, 24px) clamp(12px, 3vw, 36px) 60px' }}>
        ...
        >
          {filtered.length === 0 ? (
            ...
            <>
              {/* Desktop Table: visible on md+ (>= 768px) */}
              <div className="rec-cand-table" style={{ overflowX: 'auto' }}>
              ...
              {/* Mobile Stacked Cards: visible on < 768px */}
              <div className="rec-cand-cards" style={{ display: 'none', flexDirection: 'column', gap: 10, padding: '12px' }}>
                {filtered.map((c, i) => (
                  <motion.div
                    key={c.id}
                    ...
                    onClick={() => setSelected(c)}
                    id={`candidates-card-${c.id}`}
                    style={{
                      padding: '14px 16px', borderRadius: 16, background: 'var(--card-row-bg)',
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #3D5016, #6B8A3A)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 13, fontWeight: 800 }}>
                          {c.name.charAt(0)}
                        </div>
                        <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap', paddingTop: 2, borderTop: '1px solid var(--border)' }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)' }}>{c.appliedJobTitle}</span>
                      <StatusBadge status={c.status} />
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                      ...
                      <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>{formatDate(c.appliedAt)}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
      ...
      <style>{`
        @media (max-width: 767px) {
          .rec-cand-table { display: none !important; }
          .rec-cand-cards { display: flex !important; }
        }
      `}</style>
    </div>
  );
