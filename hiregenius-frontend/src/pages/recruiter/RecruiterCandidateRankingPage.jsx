      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38 }}>
            ...
            <h1 style={{ fontSize: 30, fontWeight: 900, color: '#fff', letterSpacing: '-0.04em', marginBottom: 6 }}>Candidate Ranking</h1>
            ...
            <div style={{ position: 'relative', display: 'inline-flex' }}>
              ...
              <select
                ...
                style={{
                  ...
                  borderRadius: 12, fontSize: 13, fontWeight: 700, minWidth: 300, cursor: 'pointer',
              ...
              {rankings.map((r, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Medal */}
                    <div style={{ width: 52, height: 52, borderRadius: 14, flexShrink: 0, background: medal?.bg ?? 'var(--card-row-bg)', border: `1px solid ${medal?.border ?? 'var(--border)'}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                      <span style={{ fontSize: 22 }}>{medal?.emoji ?? `#${r.rank}`}</span>
                    </div>

                    {/* Avatar */}
                    <div style={{ width: 44, height: 44, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #3D5016, #6B8A3A)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 15, fontWeight: 800, boxShadow: '0 3px 10px rgba(61,80,22,0.35)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '6px 14px', borderRadius: 12, background: `${scoreColor(r.matchScore)}12`, border: `1px solid ${scoreColor(r.matchScore)}25` }}>
                        <span style={{ fontSize: 26, fontWeight: 900, color: scoreColor(r.matchScore), letterSpacing: '-0.05em', lineHeight: 1 }}>{r.matchScore}%</span>
                        <span style={{ fontSize: 10, color: 'var(--text-muted)', marginLeft: 3, marginTop: 6, fontWeight: 600 }}>Match</span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '0 8px' }}>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                          <span style={{ fontSize: 18, fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.04em' }}>{r.resumeScore}</span>
                          <span style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>Resume</span>
                        </div>
                        {r.interviewScore != null && (
                          <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                            <span style={{ fontSize: 18, fontWeight: 900, color: '#60a5fa', letterSpacing: '-0.04em' }}>{r.interviewScore}</span>
