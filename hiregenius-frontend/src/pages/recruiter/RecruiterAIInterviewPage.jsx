...
const InterviewResultPanel = ({ candidateName, result, onClose }) => (
  <motion.div
    ...
    style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(12px, 3vw, 20px)' }}
  >
    ...
    <motion.div
      ...
      style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: 'min(560px, 100%)', maxHeight: '90vh', overflowY: 'auto', borderRadius: 24, background: 'var(--bg-elevated)', border: '1px solid var(--border)', boxShadow: '0 32px 96px rgba(0,0,0,0.30)' }}
    >
      {/* Header */}
      <div style={{ padding: 'clamp(14px, 3vw, 18px) clamp(16px, 3vw, 24px)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(135deg, rgba(61,80,22,0.08), transparent)', position: 'sticky', top: 0, backdropFilter: 'blur(20px)', zIndex: 2 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 11, background: 'rgba(107,138,58,0.15)', border: '1px solid rgba(107,138,58,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <button onClick={onClose} id="interview-result-close" aria-label="Close modal"
          style={{ width: 44, height: 44, minWidth: 44, minHeight: 44, borderRadius: 12, background: 'var(--card-row-bg)', border: '1px solid var(--border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}
        >
          <X size={18} />
        </button>
      </div>
      ...
      <div style={{ padding: 'clamp(16px, 3vw, 22px) clamp(16px, 3vw, 24px)' }}>
        ...
        <div style={{ padding: 'clamp(14px, 3vw, 18px) clamp(14px, 3vw, 20px)', borderRadius: 16, background: 'var(--card-row-bg)', border: '1px solid var(--border)', marginBottom: 20 }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38 }}>
            ...
            <h1 style={{ fontSize: 'clamp(22px, 4vw, 30px)', fontWeight: 900, color: '#fff', letterSpacing: '-0.04em', marginBottom: 6 }}>AI Interview</h1>
        ...
        {candidates.map((c, i) => {
          ...
          return (
            <motion.div
              ...
              className="rec-ai-card"
              style={{
                display: 'flex', alignItems: 'center', gap: 'clamp(12px, 2vw, 18px)',
                padding: 'clamp(14px, 2.5vw, 20px) clamp(14px, 2.5vw, 24px)', borderRadius: 20,
            ...
            >
              ...
              {/* Avatar */}
              <div style={{ width: 44, height: 44, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #3D5016, #6B8A3A)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 16, fontWeight: 800, boxShadow: '0 4px 12px rgba(61,80,22,0.35)' }}>
                {c.candidateName.charAt(0)}
              </div>

              {/* Info */}
              <div className="rec-ai-info" style={{ flex: 1, minWidth: 0 }}>
              ...
              {/* Status indicator */}
              <div className="rec-ai-status" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 10, background: sc.bg, flexShrink: 0 }}>
                <Icon size={13} style={{ color: sc.color }} />
                <span style={{ fontSize: 12, fontWeight: 700, color: sc.color }}>{sc.label}</span>
              </div>

              {/* Action button */}
              <div className="rec-ai-actions" style={{ flexShrink: 0 }}>
                {c.interviewStatus === 'COMPLETED' ? (
                  <button
                    ...
                    style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '10px 18px', minHeight: 44, borderRadius: 12, border: '1px solid rgba(52,211,153,0.30)', background: 'rgba(52,211,153,0.08)', color: '#34d399', fontSize: 13, fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s' }}
                  ...
                  <motion.button
                    ...
                    style={{
                      display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '10px 20px', minHeight: 44, borderRadius: 12,
