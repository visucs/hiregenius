...
const CandidateSidebar = ({ isOpen, isCollapsed, onClose, onToggleCollapse }) => {
  ...
  const content = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <nav style={{ flex: 1, overflowY: 'auto', padding: '4px 8px', scrollbarWidth: 'none' }}>
        ...
        {!isCollapsed && (
          <>
            ...
            <p style={{ fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgba(107,138,58,0.45)', padding: '2px 10px 4px' }}>Coming Soon</p>
          </>
        )}
        <div style={{
          display: 'flex', alignItems: 'center', gap: 10,
          padding: isCollapsed ? '10px 0' : '9px 12px', borderRadius: 11,
          opacity: 0.4, cursor: 'not-allowed',
          justifyContent: isCollapsed ? 'center' : 'flex-start',
        }}>
          <div style={{ width: 30, height: 30, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Mic size={16} strokeWidth={1.75} style={{ color: 'rgba(180,210,130,0.65)' }} />
          </div>
          {!isCollapsed && (
            <>
              <span style={{ fontSize: 13, color: 'rgba(180,210,130,0.65)', flex: 1 }}>Interview Practice</span>
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', padding: '2px 7px', borderRadius: 999, background: 'rgba(107,138,58,0.30)', color: 'rgba(163,230,53,0.80)', flexShrink: 0 }}>Soon</span>
            </>
          )}
        </div>
      </nav>
      ...
      {/* ── Help ─────────────────────────────────────────────── */}
      <div style={{ padding: isCollapsed ? '4px 8px' : '4px 8px', flexShrink: 0 }}>
        <div
          style={{
            display: 'flex', alignItems: 'center',
            gap: isCollapsed ? 0 : 10, padding: isCollapsed ? '10px 0' : '9px 12px',
            justifyContent: isCollapsed ? 'center' : 'flex-start',
            borderRadius: 11, cursor: 'pointer', color: 'rgba(180,210,130,0.50)',
        ...
        >
          ...
          {!isCollapsed && <span style={{ fontSize: 13, fontWeight: 500 }}>Help & Support</span>}
        </div>
      </div>
...
const CandidateTopbar = ({ onMenuToggle }) => {
  ...
  const REBUILT__ = true; // premium rebuild marker
